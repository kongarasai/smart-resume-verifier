...
import { CheckCircle, XCircle, Clock, ChevronRight, BarChart2, BookOpen, Star } from 'lucide-react';
...
export default function PracticePage() {
  ...
  const handleCategoryClick = (cat: string) => {
    if (cat === 'coding') {
       window.location.href = '/candidate/practice/coding';
    }
        ...
        {phase === 'select' && (
          <>
            <div className="flex items-center justify-between mb-8">
              ...
              <button onClick={() => window.location.href='/candidate/progress'} className="btn-secondary"><BarChart2 size={14} /> My Progress</button>
            </div>
            ...
            {data.groups.length > 0 && (
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {data.groups.map(g => (
                    <a key={g.id} href={`/candidate/assignments/${g.id}`} 
          ...
          <div className="max-w-lg mx-auto text-center py-12 animate-slide-up">
            ...
            <div className="card p-6 my-6 text-left">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-center">
                <div><div className="font-display text-3xl text-ink-900">{correct}</div><div className="text-xs text-ink-500">Correct</div></div>
                <div><div className="font-display text-3xl text-ink-900">{questions.length - correct}</div><div className="text-xs text-ink-500">Incorrect</div></div>
                <div><div className="font-display text-3xl text-ink-900">{Math.round((correct / questions.length) * 100)}%</div><div className="text-xs text-ink-500">Score</div></div>
              </div>
            </div>
            <div className="flex gap-3 justify-center">
              <button onClick={() => setPhase('select')} className="btn-primary">Practice More</button>
              <button onClick={() => window.location.href='/candidate/progress'} className="btn-secondary">View Progress</button>
            </div>
          </div>
        )}
