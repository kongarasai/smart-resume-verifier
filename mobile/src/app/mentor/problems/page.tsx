...
export default function MentorProblemsPage() {
  ...
  const saveBulk = async () => {
    ...
    if (bulkQuestions.length === 0) return toast.error('No questions to save');
    setSaving(true);
    try {
                      ...
                      <button 
                        onClick={async () => {
                          ...
                          try {
                            ...
                            mentorAPI.getGroupQuestions(aiGeneratedData.groupId).then((r: any) => setGroupQuestions(r.data || []));
                          } catch {
                            toast.error('Saving failed');
                          } finally {
                            setSaving(false);
              ...
              <div className="space-y-6">
                <div className="card p-6">
                  ...
                  <div className="flex flex-col items-center justify-center border-2 border-dashed border-ink-200 rounded-xl p-8 bg-ink-25/50">
                    <input type="file" id="bulk-pdf-mentor" className="hidden" accept=".pdf" onChange={handlePdfParse} />
                    <label htmlFor="bulk-pdf-mentor" className="btn-secondary py-3 px-6 cursor-pointer flex flex-col items-center gap-2">
                      {parsingPdf ? (
                        <>
